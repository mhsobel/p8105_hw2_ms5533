...
# most observed state --> NO IDEA, TRYING TO FIND MOST FREQUENT...

```

There are **`r nrow(distinct(brfss_data, locationabbr, locationdesc, .keep_all = FALSE))` unique locations** in this dataset describing "Overall Health" and **every state is represented, INCLUDING AN EXTRA ONE??**. The most observed state is **UHHHHH...**. 

2. _*In 2002, what is the median of the “Excellent” response value?*_

```{r, eval=FALSE}
# group by year, filter by 2002, sum -> median of "excellent"
group_by(brfss_data, year) %>% 
  filter(year == 2002) %>% 
  summarise(median(excellent)) # VALUES NOT FOUND???
# CODE NOT RUNNING --> NA
```
...
```{r, eval=FALSE}
# group by year in data set, filter by 2002, select excellent --> hist
group_by(brfss_data, year) %>% 
  filter(year == 2002) %>%
    select(year, excellent) %>% 
  hist(excellent)
# NOT SURE WHAT ELSE TO DO...SEEMS TO BE GOOD --> 'x' not numeric? 
```
